...
local HvHSettings = getgenv().HvHSettings

if HvHSettings == nil then
...
local Library = Module.Functions.HttpGet("https://raw.githubusercontent.com/thtduddawithmiclo12ud2222/Ui/main/pxxxfix")
Library.theme.accentcolor = Color3.fromRGB(238, 130, 238)
Library.theme.accentcolor2 = Color3.fromRGB(255, 20, 147)
...
if HvHSettings.Watermark == true then

Library:CreateWatermark("HvH.Me | {fps} | {game}")

end

local Window = Library:CreateWindow(Script.Name, Vector2.new(492, 598), Enum.KeyCode.V)
local LegitTab = Window:CreateTab("Rage")
local AimbotSection = LegitTab:CreateSector("Aimlock", "left")

AimbotSection:AddToggle('Enabled', false, function(State)
    Aimlock = State
end)

AimbotSection:AddToggle('Auto Prediction', false, function(State)
    getgenv().AutoPrediction = State
end)

AimbotSection:AddToggle('Airshot Function', false, function(State)
    getgenv().CheckIfJumped = State
end)

AimbotSection:AddToggle('Visible Check', false, function(State)
    getgenv().VisibleCheck = State
end)

AimbotSection:AddDropdown('Hitbox', {"Head", "UpperTorso", "HumanoidRootPart", "LowerTorso"}, "Head", false, function(Option)
...
local FOVSection = RageTab:CreateSector("Silent Fov", "right")
...
local AntiAimSeciton = LegitTab:CreateSector("Anti Aim", "right")
...
local WhitelistSection = RageTab:CreateSector("Settings", "left")
...
MovementSection:AddLabel("Another CFrame Speed")

local L_55_ =  MovementSection:AddButton("CFrame Speed (N)", function()
	repeat
		wait()
	until game:IsLoaded()
	local gsp = game:service('Players')
	local glp = gsp.LocalPlayer
	repeat
		wait()
	until glp.Character
	local gsuis = game:service('UserInputService')
	local gsrs = game:service('RunService')
	getgenv().Multiplier = 0.5
	local Enabled = true
	local Plr
	gsuis.InputBegan:connect(function(Key)
		if Key.KeyCode == Enum.KeyCode.LeftBracket then
			Multiplier = Multiplier + 0.01
			print(Multiplier)
			wait(0.2)
			while gsuis:IsKeyDown(Enum.KeyCode.LeftBracket) do
				wait()
				Multiplier = Multiplier + 0.01
				print(Multiplier)
			end
		...
		if Key.KeyCode == Enum.KeyCode.RightBracket then
			...
			print(Multiplier)
			wait(0.2)
			while gsuis:IsKeyDown(Enum.KeyCode.RightBracket) do
				wait()
				Multiplier = Multiplier - 0.01
				print(Multiplier)
			end
...
local L_56_ = MovementSection:AddSlider("Speed", 0, 1, 5, 1, function(speed1)
	getgenv().Multiplier = speed1
end)

MovementSection:AddTextbox("Custom Speed", "", function(speed2)
	getgenv().Multiplier = speed2
end)
...
local MainSection = LegitTab:CreateSector("Settings", "left")
...
if not HvHSettings.PerformanceMode then
